...
the spec already answers.`,
      ...
      },
      {
        ...
        blocks: [
          {
            type: 'text',
            md: "Two smaller habits pay for themselves many times over.\n\nFirst, **decompose before you generate**. That means splitting one big request into two conversations: one about the plan, then one about the product. Say you need a script that migrates customer data to a new database. The tempting move is a single giant prompt: 'write me the migration script.' Hold off. Start with 'give me two or three ways you'd approach this migration, with the tradeoffs of each.' Read the options. Poke at the weak spots: 'what happens with approach B if the table has ten million rows?' Once an approach survives your questions, ask for the code: 'write the script using approach B.' Two extra minutes of planning saves the half hour you'd spend untangling code built on the wrong plan. Claude Code's plan mode bakes this same habit into the product.\n\nWhy can't extended thinking save you here? Because thinking harder and choosing the goal are two different jobs. Extended thinking makes the model better at executing the task exactly as you worded it. Whether you asked for the right thing in the first place never gets examined. Ask the wrong question with thinking cranked up and you get a beautifully reasoned answer to the wrong question. Deciding what to ask stays your job.\n\nSecond, **edit rather than argue**. When Claude gets something wrong, the natural instinct is to type a follow-up: 'no, I meant...'. Resist it. Every correction round leaves the failed attempt sitting in the context window, and you saw in [Mental Models · How LLMs Actually Work](lesson:m0-l1) that the model pays attention to everything in that window, including the garbage. The better move: go back, fold what you learned into the original prompt, and regenerate. The diagram below shows the difference.",
          },
          {
            type: 'diagram',
